...
class SwapLocalOptimizer(LocalOptimizer):
    ...
    def optimize(self, initial: set, G: nx.Graph, max_steps: int) -> set:
        ...
        for i in range(max_steps):
            ...
            add_index = np.argmin([float('inf') if j in self.subset else self.cross_edges[j] for j in G.nodes])
            ...
            rem_index = np.argmax([-1 if j not in self.subset else self.cross_edges[j] for j in G.nodes])

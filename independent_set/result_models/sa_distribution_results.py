...
class SADistributionResults(Result):
    ...
    def __init__(self, G: nx.Graph, planted_ind_set: set[int], epsilon: int, num_trials: int, headstart_size: int):
        super().__init__("sa-distribution")
        # Store metadata
        self.G: nx.Graph = G
        ...
        self.final_sizes: List[int] = [-1] * self.num_trials
    ...
    def add_result(self, vertex_permutation: List[int], solution: set):
        self.vertex_permutations.append(vertex_permutation)
        self.final_sizes.append(len(solution))
        self.final_intersections.append(len(set(self.planted_ind_set).intersection(solution)))
    ...
    def get_final_sizes_minus_intersections(self) -> List[int]:
        ...
        """
        return [self.final_sizes[t] - self.final_intersection[t] for t in range(self.num_trials)]

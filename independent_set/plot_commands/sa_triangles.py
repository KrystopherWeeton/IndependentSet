...
INTERSECTION_FORMATTING: Formatting = Formatting(
...
TRIANGLE_FORMATTING: Formatting = Formatting(
    style="-", width="1", color="orange", alpha=0.5
)
...
def __probability_of_good_move_estimator(
    omega: int, n: int, s: int, k: int, epsilon: int
) -> float:
    if s == k:
        pg: float = omega / (2 * n)
...
def __expected_height(
    omega: int, n: int, s: int, k: int, epsilon: int, t: int
) -> float:
...
def plot_sa_triangles(today, file_name, transient):
    ...
    plot.initialize_figure(
        "Subset Size (s)", "Intersection Size", "Size vs. Intersection", (40, 16)
    )

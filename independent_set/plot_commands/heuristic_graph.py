...
def plot_heuristics_graphs(today, folder):
    results: HeuristicResults = verify_and_load_results_v2(HeuristicResults, "independent_set", today)
    n_values: List[int] = results.get_n_values()

    intersection_sizes: List[StatInfo] = results.get_all_intersection_size_info()
    densities: List[StatInfo] = results.get_all_density_info()
    subset_sizes: List[StatInfo] = results.get_all_subset_size_info()

    intersection_means: List[float] = [x.mean for x in intersection_sizes]

    planted_sizes = [
        results.planted_sizes[n] for n in n_values
    ]

    intersection_data: List[List[float]] = results.get_intersection_data()
    subset_data: List[List[float]] = results.get_subset_size_data()
